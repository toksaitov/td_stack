...
#define TD_MAIN_EXIT_WITH_FAILURE \
    ...
    *path_state = (td_path_state) {
    ...
    td_array_stack_push(path, path_state);

    srand(RANDOM_SEED);

    for (;;) {
        while (!td_array_stack_is_empty(path) &&
                    ((td_path_state *) td_array_stack_top(path))->
                                           pending_directions ==
                                               TD_DIRECTION_NOWHERE) {
            td_path_state *path_state =
                td_array_stack_pop(path);
            ...
            if (path_state) {
                ...
                if (!td_array_stack_is_empty(path)) {
                    path_state =
                        td_array_stack_top(path);
...
cleanup:
    ...
    td_array_stack_free_with_elements(path);
    path = NULL;
